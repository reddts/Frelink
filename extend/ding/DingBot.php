...
class DingBot
{
     ...
     */
    public function setConfig(array $config = []): DingBot
    {
    ...
    /**
     * 发送给手机号
     * @param  string $mobiles ,array  $mobiles 接收人手机号
     * 接收三种类型传参:
     * 1:139xxxxx163
     ...
     * @return $this
     */
    public function at($mobiles): DingBot
    {
        if (empty($mobiles)) {
            return $this;
        }
     ...
     * @return bool
     */
    public function link(string $title, string $content, string $linkUrl, string $picUrl = ''): bool
    {
        $this->msgType = 'link';
        $data          = [
            'link' => [
     ...
     */
    public function markdown(string $title, string $content): bool
    {
     ...
     * @param int $btnOrientation 按钮排列 0:竖排,1:横排
     * @param int $hideAvatar 头像显示 0:隐藏,1:显示
     * @return bool [type]                      [description]
     */
     ...
     * @return DingBot [type]        [description]
     */
    public function makeBtn(string $title, string $url): DingBot
    {
        $this->btns[] = [
            'title'     => $title,
            'actionURL' => $url,
     ...
     * @return $this
     */
    public function makeLink(string $title, string $msgUrl, string $picUrl): DingBot
    {
        $this->links[] = [
            'title'      => $title,
            'messageURL' => $msgUrl,
     ...
     * @return bool       发送结果
     */
    public function sendMsg(array $data): bool
    {
