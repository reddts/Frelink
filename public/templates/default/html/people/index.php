            ...
            <div class="swiper-container bg-white">
                <ul class="aw-pjax-tabs nav nav-tabs nav-tabs-block px-3 bg-white swiper-wrapper" style="flex-wrap: nowrap;">
                    ...
                    <li class="nav-item swiper-slide {if $type=='question'}i-active{/if}">
                        <a class="nav-link {if $type=='question'}active{/if}" data-pjax="tabMain" href="{:url('people/index',['name'=>$user['url_token'],'type'=>'question'])}">{:L('FAQ')} {$question_count}</a>
                    </li>
                    <li class="nav-item swiper-slide {if $type=='answer'}i-active{/if}">
                        <a class="nav-link {if $type=='answer'}active{/if}" data-pjax="tabMain" href="{:url('people/index',['name'=>$user['url_token'],'type'=>'answer'])}">{:L('补充')} {$answer_count}</a>
                    </li>
                    <li class="nav-item swiper-slide {if $type=='article'}i-active{/if}">
                        <a class="nav-link {if $type=='article'}active{/if}" data-pjax="tabMain" href="{:url('people/index',['name'=>$user['url_token'],'type'=>'article'])}">{:L('内容')} {$article_count}</a>
                    </li>
