...
Code.prototype = {
    ...
    _createPanel: function _createPanel(value) {
        ...
        if (editor.customConfig.codeType != null) {
            var select = "<div class='codeType' style='margin-bottom: 10px;'><select class='form-control'>";
    ...
    _createPanel: function _createPanel() {
        ...
        var tabsConfig = [
            ...
            }, // first tab end
            {
                ...
                title: '插入视频',
                // 模板
                tpl: '<div><input id="' + textValId + '" type="text" class="block" placeholder="请输入网页链接..."/><div class="w-e-button-container"><button id="' + btnId + '" class="right">\u63D2\u5165</button></div></div>',
                // 事件绑定
                events: [{
                    selector: '#' + btnId,
                    type: 'click',
                    fn: function fn() {
                        var $text = $('#' + textValId);
                        var val = $text.val().trim();
                        if (val) _this._insert(`<iframe src="${val}"></iframe>`); // 插入视频
                        // 返回 true，表示该事件执行完之后，panel 要关闭。否则 panel 不会关闭
                        return true;
                    }
                }]
...
UploadImg.prototype = {
    ...
    insertLinkVideo: function insertLinkVideo(link) {
        ...
        editor.cmd.do('insertHTML', '<iframe src="' + link + '" style="max-width: 100%;height: auto" frameborder="0" allowfullscreen="true" allowtransparency="true"></iframe>');
    }
};
