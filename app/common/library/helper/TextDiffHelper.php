...
class TextDiffHelper
{
    ...
    public static function compare($string1, $string2, $compareCharacters = false): array
    {
        ...
        if ($compareCharacters) {
            $sequence1 = $string1;
            $sequence2 = $string2;
            $end1 = cjk_strlen($string1) - 1;
            $end2 = cjk_strlen($string2) - 1;
        } else {
        ...
        for ($index = $end1 + 1; $index < ($compareCharacters ? cjk_strlen($sequence1) : count($sequence1)); $index++) {
            $diff[] = array($sequence1[$index], self::UNMODIFIED);
        }
        return $diff;
    }
