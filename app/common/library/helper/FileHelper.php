...
class FileHelper
{
    ...
    */
     ...
     * @param $path
     * @param string $type
     * @param bool $ext 是否包含后缀
     * @return array
     */
    public static function getFileList($path, string $type='', bool $ext=true): array
    {
        $list = [];
                ...
                if (is_dir($path . "/" . $file)) {
                    ...
                    if($type==''){
                        ...
                        if (!$ext)
                        {
                            ...
                            $list[] = str_replace('.'.$fileInfo['extension'],'',$file);
                        }else{
                            $list[] = $file;
                        }
                    }else{
                        ...
                        if(strpos($fileType,$type)!==false){
                            ...
                            {
                                $fileInfo = pathinfo($path.'/'.$file);
                                $list[] = $path.'/'.str_replace('.'.$fileInfo['extension'],'',$file);
                            }else{
                                $list[] = $path.'/'.$file;
                            }
                        }
	...
	public function getChildren()
	{
		$current = $this->current($this->_values);
		if ($current['isDir']) {
			return self::getFiles($current['pathname']);
		}
		return false;
	}
	 ...
	 * @return bool
	 */
	public static function delDir($directory,bool $subDir = true)
	{
		if (!is_dir($directory)) {
			return false;
		}
