...
<section class="p-3 admin-page-shell">
    ...
    <div class="container-fluid">
        ...
        <div class="card admin-panel">
            <div class="card-body">
                <form class="form-horizontal admin-form-layout" method="{$form_method}" action="{$form_url}" id="awFormBuilder">
                    {if $form_items}
                    ...
                    {/switch}
                    {/volist}
                    <div class="form-group admin-form-actions mb-0">
                        {php}if(isset($btn_hide) && !in_array('submit', $btn_hide)):{/php}
                        <button type="{$form_method=='post'?'button':'submit'}" class="btn btn-primary btn-lg {$form_method=='post'?'aw-ajax-form':''}" {if $submit_confirm} data-confirm="{:L('是否确认提交该表单')}？"{/if}>{$btn_title['submit']|default=L('提交')}</button>
                        {php}endif;{/php}
