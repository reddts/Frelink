<ul class="nav nav-tabs nav-tabs-block" id="builder-form-group-tab" role="tablist" data-toggle="tabs">
    ...
    <li class="nav-item">
        <a class="nav-link {eq name="items_key" value="1" }active{/eq}" data-bs-toggle="tab" id="nav-tab-{$items_key}" href="#nav-tab-content-{$items_key}" role="tab" data-toggle="tab">{:L($key)}</a>
    </li>
    {/volist}
</ul>
<div class="tab-content p-3">
    {volist name="form.options" id="items" key="items_key"}
    <div class="tab-pane {eq name="items_key" value="1"}active{/eq}" id="nav-tab-content-{$items_key}" aria-labelledby="nav-tab-{$items_key}">
