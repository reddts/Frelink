...
- `GET /api/Insight/weekly_execution?days=7&limit=3`
- `GET /api/Insight/weekly_execution?days=7&limit=3&format=markdown`
...
- `GET /api/Insight/weekly_execution` 返回可直接交给站长或 agent 的本周执行清单，包含建议标题、原因和动作入口；`format=markdown` 时返回可直接复制的文本周报。

### `weekly_execution` 返回结构补充
...
- 顶层字段：
  ...
  - `tasks`
- `tasks` 中每条任务固定包含：
  - `task_id`：当前周执行任务的稳定标识，可用于执行追踪
  - `dedupe_key`：去重键，可用于避免重复创建同一类任务
  - `task_type`：当前固定为 `fill_gap`
  - `status`：当前固定为 `pending`
  - `suggested_owner`：建议由谁处理，如 `research_editor / observer / knowledge_editor / faq_editor`
  - `window_start / window_end / expires_at`：任务所属时间窗口与失效时间
  - `content_type`：如 `research / fragment / tutorial / faq`
  - `source_key`：来自哪类缺口，如 `research / fragment / faq / help`
  - `priority`：`high / medium / low`
  - `label / title / keyword / reason`
